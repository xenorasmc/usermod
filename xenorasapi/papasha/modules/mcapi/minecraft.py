...
class Minecraft:
    ...
    def xsCall(self, player):
        """Post a message to the game chat"""
        self.conn.send(b"chat.newxs", player)

    def addwhitelist(self, name):
        self.conn.send(b"whitelist.add", name)
    ...
    def namefromid(self, uuid):
        return self.conn.sendReceive(b'adv.namefromid', uuid)

    @staticmethod
    def create(address = "localhost", port = 4444, salt = "salt"):
        return Minecraft(Connection(address, port, salt))